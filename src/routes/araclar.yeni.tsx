...
function VehicleOperationForm() {
  ...
  useEffect(() => {
    ...
    if (stored) { try { const draft = JSON.parse(stored) as Draft; if (Date.now() - draft.savedAt < 86_400_000) {
      setPhone(draft.phone); setFirstName(toBusinessUppercase(draft.firstName)); setLastName(toBusinessUppercase(draft.lastName)); setCustomerNote(toBusinessUppercase(draft.customerNote)); setBrand(toBusinessUppercase(draft.brand)); setModel(toBusinessUppercase(draft.model));
      setOperationType(draft.operationType); setDescription(toBusinessUppercase(draft.description)); setPrice(draft.price); setPaymentMethod(draft.paymentMethod);
      setMultimediaProductId(draft.multimediaProductId); setScreenProductId(draft.screenProductId); setSupplierId(draft.supplierId); setOperationNote(toBusinessUppercase(draft.operationNote)); setOperationAt(draft.operationAt);
      initialized.current = contextKey; return;
    } } catch { /* discard invalid draft */ } sessionStorage.removeItem(key); }
    const detail = detailQuery.data;
    setPhone(context.customer?.phone ?? ""); setFirstName(toBusinessUppercase(context.customer?.firstName ?? "")); setLastName(toBusinessUppercase(context.customer?.lastName ?? "")); setCustomerNote(toBusinessUppercase(context.customer?.note ?? "")); setBrand(toBusinessUppercase(context.vehicle.brand ?? "")); setModel(toBusinessUppercase(context.vehicle.model ?? ""));
    if (detail) { setOperationType(detail.operationType ?? "OTHER"); setDescription(toBusinessUppercase(detail.description)); setPrice(detail.price); setPaymentMethod(detail.paymentMethod); setMultimediaProductId(detail.multimediaProductId ?? ""); setScreenProductId(detail.screenProductId ?? ""); setSupplierId(detail.mailOrderSupplierId ?? ""); setOperationNote(toBusinessUppercase(detail.note ?? "")); setOperationAt(toLocalDateTime(detail.operationAt)); }
    initialized.current = contextKey;
  }, [context, contextKey, detailQuery.data]);
  useEffect(() => { if (offerQuery.data) { setOperationType("SOUND_SYSTEM"); setPrice(offerQuery.data.finalTotal); setDescription((value) => value || "SES SİSTEMİ"); } }, [offerQuery.data]);
    ...
    <form onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }} className="grid gap-4 lg:grid-cols-3">
      <div className="card-elevated space-y-4 p-6 lg:col-span-2">
        <h2 className="font-bold">Araç ve Müşteri Bilgileri</h2><div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Ad"><Input businessText readOnly={isEdit} value={firstName} onChange={(e) => setFirstName(e.target.value)} className={inputClass} /></Field><Field label="Soyad"><Input businessText readOnly={isEdit} value={lastName} onChange={(e) => setLastName(e.target.value)} className={inputClass} /></Field>
          <Field label="Araç Markası"><Input businessText readOnly={isEdit} value={brand} onChange={(e) => setBrand(e.target.value)} className={inputClass} /></Field><Field label="Model"><Input businessText readOnly={isEdit} value={model} onChange={(e) => setModel(e.target.value)} className={inputClass} /></Field>
        </div>{!isEdit && <Field label="Müşteri Notu"><Textarea businessText value={customerNote} onChange={(e) => setCustomerNote(e.target.value)} rows={2} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" /></Field>}
        <hr className="border-border/60" /><h2 className="font-bold">İşlem Detayı</h2><div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="İşlem Açıklaması (İsteğe bağlı)"><Input businessText maxLength={150} value={description} onChange={(e) => setDescription(e.target.value)} className={inputClass} /></Field>
        ...
        <Field label="İşlem Notu"><Textarea businessText value={operationNote} onChange={(e) => setOperationNote(e.target.value)} rows={3} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm" /></Field>
      </div>
