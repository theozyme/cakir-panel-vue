...
import { activeSupplierPayment, nextSupplierPayment, preparePaymentRate, serializeSupplierPayment, supplierStateKey } from "./payment-state.js";
import { istanbulDate } from "./payment-conversion.js";
...
const formatDateKey = istanbulDate;
...
const getDayRange = (dateKey: string) => { const start = new Date(`${dateKey}T00:00:00+03:00`); return { start, end: new Date(start.getTime() + 86_400_000) }; };
const parseOperationAt = (value: unknown) => { if (typeof value !== "string" || !/T.*(?:Z|[+-]\d{2}:\d{2})$/.test(value)) throw new HttpError(400, "operationAt zaman dilimli ISO tarih olmali"); const parsed = new Date(value); if (Number.isNaN(parsed.getTime())) throw new HttpError(400, "operationAt gecersiz"); if (parsed.getTime() > Date.now() + 1000) throw new HttpError(400, "operationAt gelecekte olamaz"); return parsed; };
...
export const createVehicleOperation = async (body: unknown): Promise<CreateVehicleOperationResponse> => {
  ...
  try { return await withSerializableTransaction(async (tx) => {
    ...
    const payment = await nextSupplierPayment(tx, input.operation.mailOrderSupplierId, price, currency, operationAt, rate);
    if (payment) await createVehicleOperationSupplierPayment({ tx, operationId: operation.id, ...payment });
...
const supplierState = async (tx: BusinessTransaction, row: any) => {
  const active = await activeSupplierPayment(tx, row.id);
  if (row.paymentMethod === "MAIL_ORDER" && (!active || active.supplierId !== row.mailOrderSupplierId)) throw new HttpError(409, "Operation supplier hareketi eksik veya uyusmuyor");
  if (row.paymentMethod !== "MAIL_ORDER" && active) throw new HttpError(409, "Beklenmeyen aktif supplier hareketi");
  return active;
};
...
export const updateVehicleOperation = async (id: string, body: unknown): Promise<VehicleOperationDetail> => {
  ...
  const correction = root.correction === undefined ? null : asRecord(root.correction, "correction");
  const correctionReason = correction ? requiredString(correction.reason, "correction.reason") : null;
  if (correction) { nextInput.priceInput = correction.originalAmountTry; if (nextInput.currency !== "TRY") throw new HttpError(400, "TL duzeltmesinde currency TRY olmali"); }
  const rate = await preparePaymentRate(nextInput.mailOrderSupplierId, nextInput.currency, nextOperationAt, id, asRecord(root.operation).expectedExchangeRate);
  try { await withSerializableTransaction(async (tx) => {
    const current = await lockActiveOperation(tx, id, revision);
    const previousSupplier = await supplierState(tx, current);
    if (correction && (current.currency !== "USD" || current.paymentMethod !== "MAIL_ORDER" || previousSupplier?.sourceAmount != null || nextInput.paymentMethod !== current.paymentMethod || nextInput.mailOrderSupplierId !== current.mailOrderSupplierId || nextInput.type !== current.operationType || nextInput.multimediaProductId !== current.multimediaProductId || nextInput.screenProductId !== current.screenProductId || nextInput.soundOfferId !== current.soundOfferId || nextOperationAt.getTime() !== current.operationAt.getTime())) throw new HttpError(400, "TL duzeltmesi yalnizca eski USD mail order kaydina, tarih ve stok baglantilari korunarak uygulanabilir");
    if (!correction && nextInput.currency !== current.currency) throw new HttpError(400, "Para birimi degisikligi icin acik TL duzeltmesi gerekli");
    const beforeJson = { ...(snapshotOperation({ ...current, supplierPayment: serializeSupplierPayment(previousSupplier) }) as object), supplierPayment: serializeSupplierPayment(previousSupplier), ...(correctionReason ? { correctionReason } : {}) } as Prisma.InputJsonValue;
    ...
    const nextSupplier = await nextSupplierPayment(tx, nextInput.mailOrderSupplierId, price, currency, nextOperationAt, rate);
    ...
    const after = await tx.vehicleOperation.findUniqueOrThrow({ where: { id }, select: operationSelect }); await tx.vehicleOperationRevision.create({ data: { operationId: id, action: "UPDATE", fromRevision: revision, toRevision: revision + 1, beforeJson, afterJson: { ...(snapshotOperation({ ...after, supplierPayment: serializeSupplierPayment(await activeSupplierPayment(tx, id)) }) as object), supplierPayment: serializeSupplierPayment(await activeSupplierPayment(tx, id)) } as Prisma.InputJsonValue } });
...
export const deleteVehicleOperation = async (id: string, body: unknown): Promise<DeleteVehicleOperationResponse> => {
  ...
  return withSerializableTransaction(async (tx) => { const current = await lockActiveOperation(tx, id, revision); const previousSupplier = await supplierState(tx, current); const beforeJson = { ...(snapshotOperation({ ...current, supplierPayment: serializeSupplierPayment(previousSupplier) }) as object), supplierPayment: serializeSupplierPayment(previousSupplier) } as Prisma.InputJsonValue;
    ...
    if (previousSupplier) await reconcileVehicleOperationSupplierPayment({ tx, operationId: id, previous: previousSupplier, next: null });
